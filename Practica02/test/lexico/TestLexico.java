...
public class TestLexico {
	...
	public static void main(String args[]) throws IOException {
		...
		while ((token = lexico.yylex()) != 0) {
			expectedTokens[i].assertEqual(lexico.getLine(), lexico.getColumn(),
					token, lexico.getYylval());
			i++;
		}
	}
	...
	private static LexicalInfo[] expectedTokens = new LexicalInfo[] {
			...
			new LexicalInfo(16, 6, Parser.CTE_ENTERA, 10),
			...
			new LexicalInfo(20, 6, Parser.CTE_ENTERA, 1),
			new LexicalInfo(20, 7, ';', null),
			new LexicalInfo(21, 2, Parser.ID, "r"),
			new LexicalInfo(21, 4, '=', null),
			new LexicalInfo(21, 6, Parser.CTE_REAL, 12.3),
			new LexicalInfo(21, 11, '-', null),
			new LexicalInfo(21, 13, Parser.CTE_REAL, 0.03412),
			new LexicalInfo(21, 22, '*', null),
			new LexicalInfo(21, 24, Parser.CTE_REAL, 2.0),
			new LexicalInfo(21, 27, '+', null),
			new LexicalInfo(21, 29, Parser.CTE_REAL, 0.34),
			new LexicalInfo(21, 33, '-', null),
			new LexicalInfo(21, 35, Parser.CTE_REAL, 3000.0),
			new LexicalInfo(21, 38, ';', null),
			new LexicalInfo(22, 2, Parser.ID, "c"),
			new LexicalInfo(22, 4, '=', null),
			new LexicalInfo(22, 6, Parser.CTE_CARACTER, '~'),
			...
			new LexicalInfo(24, 8, Parser.CTE_ENTERA, 2),
			new LexicalInfo(24, 9, ']', null),
			new LexicalInfo(24, 10, ';', null),
			new LexicalInfo(25, 2, Parser.IF, null),
			new LexicalInfo(25, 5, '(', null),
			new LexicalInfo(25, 6, Parser.CTE_ENTERA, 1),
			new LexicalInfo(25, 8, Parser.Y, null),
			new LexicalInfo(25, 11, Parser.CTE_ENTERA, 2),
			new LexicalInfo(25, 13, Parser.O, null),
			new LexicalInfo(25, 16, '!', null),
			new LexicalInfo(25, 17, Parser.CTE_ENTERA, 0),
			...
			new LexicalInfo(29, 11, Parser.CTE_ENTERA, 10),
			...
			new LexicalInfo(30, 10, Parser.CTE_ENTERA, 1),
...
class LexicalInfo {
	...
	public String toString() {
		...
		sb.append("(").append(this.line).append(", ").append(this.column)
				.append(", ").append(this.token).append(", ")
				.append(this.semanticValue).append(")");
		return sb.toString();
	}

	public void assertEqual(int line, int column, int token,
			Object semanticValue) {
		assert this.line == line : "Assert failed in token " + this
				+ ". Expected line " + this.line + ", obtained " + line + ".";
		assert this.column == column : "Assert failed in token " + this
				+ ". Expected column " + this.column + ", obtained " + column
				+ ".";
		assert this.token == token : "Assert failed in token " + this
				+ ". Expected token " + this.token + ", obtained " + token
				+ ".";
		assert this.semanticValue == null ? true
				: this.semanticValue
						.equals(semanticValue) : "Assert failed in token "
								+ this + ". Expected semanticValue "
								+ this.semanticValue + ", obtained "
								+ semanticValue + ".";
	}
}
