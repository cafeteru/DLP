...
public class Invocacion extends SentenciaAbstracta implements Expresion {
	...
	public List<Tipo> examen(int numParametros) {
		List<Tipo> tipos = new ArrayList<Tipo>();
		TipoFuncion f = (TipoFuncion) getVariable().getDefinicion().getTipo();
		boolean existe = false;
		List<ListaExamen> lista = new ArrayList<ListaExamen>();

		for (int i = 0; i < getListaExamen().size(); i++) {
			existe = false;
			ListaExamen a = (ListaExamen) getListaExamen().get(i);
			for (DefVariable b : f.getParametros()) {
		...
		for (DefVariable a : f.getParametros()) {
			...
			if (existe == false) {
				Expresion l;
				if (a.getTipo() instanceof TipoCaracter)
					l = new LiteralCaracter(getLinea(), getColumna(), '0');
				else
					l = new LiteralEntero(getLinea(), getColumna(), 0);
				getExpresiones().add(l);
				tipos.add(a.getTipo());
			}
		}
		return tipos;
