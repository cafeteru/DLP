...
argumentosLlamada: expresionesExamen 																	{ 	$$ = $1;}
				 | /*vacio*/  																			{ 	$$ = new ArrayList<ListaExamen>();	}
				 ;

expresionesExamen: expresionExamen ',' expresionesExamen												{ 	$$ = $3; ((List<ListaExamen>)$$).add((ListaExamen)$1);	}
				| expresionExamen																		{ 	$$ = new ArrayList<ListaExamen>(); ((List<ListaExamen>)$$).add((ListaExamen)$1);	}
				;
				
expresionExamen: ID ':' constante                               										{ 	$$ = new ListaExamen(lexico.getLine(), lexico.getColumn(), (String)$1 , (Expresion)$3);}	
				;
constante: CTE_ENTERA                               													{ 	$$ = new LiteralEntero(lexico.getLine(), lexico.getColumn(), (Integer) $1);		}
         | CTE_REAL 																					{ 	$$ = new LiteralReal(lexico.getLine(), lexico.getColumn(), (Double) $1);	}
         | CTE_CARACTER 																				{ 	$$ = new LiteralCaracter(lexico.getLine(), lexico.getColumn(), (Character)$1);		}
		 ;
